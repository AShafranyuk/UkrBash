...
- (NSString *)confirmButtonTitle {
    if (!_confirmButtonTitle) {
        _confirmButtonTitle = NSLocalizedString(@"OK", @"HSDatePicker confirm button title");
    }
...
- (NSString *)backButtonTitle {
    if (!_backButtonTitle) {
        _backButtonTitle = NSLocalizedString(@"Terug", @"HSDatePicker back button");
    }
...
- (NSString *)stringDateForRow:(NSUInteger)row {
    ...
    if ([self isDate:date sameDayAsDate:[NSDate date]]) {
        return NSLocalizedString(@"Vandaag", @"Current day indicator");
    }
